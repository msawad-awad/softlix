  ...
  app.post("/api/webhook/google-ads-leads", async (req, res) => {
    try {
      ...
      const formLead = await storage.createFormLead({
        ...
        phone: phone || null,
        message: [
          body.campaign_id ? `الحملة: ${body.campaign_id}` : null,
          body.adgroup_id ? `المجموعة الإعلانية: ${body.adgroup_id}` : null,
          body.form_id ? `نموذج الإعلان: ${body.form_id}` : null,
          body.gcl_id ? `GCL ID: ${body.gcl_id}` : null,
          companyName ? `الشركة: ${companyName}` : null,
          city ? `المدينة: ${city}` : null,
        ].filter(Boolean).join(" | ") || "إعلانات جوجل",
